...
class SIRCellularAutomataSimple:
    ...
    def update_grid(self):
        # ★★★ 修正箇所: Cellオブジェクトの操作からNumPy配列の操作へ ★★★
        
        # GridUpdaterのupdate_gridに、必要なNumPy配列を渡す
        self.grid, self.infection_time = self.grid_updater.update_grid(
            self.grid,                # Cellオブジェクトのリスト
            self.infection_time,
            self.get_neighbors,       # numba関数では使用しないが引数として残す
            self.recovery_time,
            self.infection_probability,
            self.cell_size_m
        )
        
        # GridUpdater内で更新されたCellオブジェクトの状態を、state_gridに反映させる
        # これがシミュレーションの高速化を実現します。
    ...
    def visualize(self, time_step, t_end, ax1):
        ...
        for i, j in active_coords:
            ...
            # ★★★ 修正箇所: GridUpdaterの静的メソッドとして呼び出す ★★★
            burn_intensity = GridUpdater.active_function(t, n) 
