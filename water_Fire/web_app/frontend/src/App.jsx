...
function App() {
  ...
  const fetchElevation = async () => {
    try {
      const res = await axios.get(`${API_URL}/elevation`);
      setElevationData(res.data.elevation);
    } catch (err) {
  ...
  const resetSimulation = async () => {
    try {
      ...
      fetchState();
      fetchElevation(); // Fetch elevation on reset (or init)
      setIsRunning(false);
    } catch (err) {
      ...
      <div className="flex gap-4 w-full max-w-7xl">
        ...
        <div className="flex-1 flex gap-4">
          ...
          <div className="flex-1 bg-slate-800 rounded-xl p-4 shadow-2xl border border-slate-700 relative">
            ...
            {/* Stats Overlay */}
            <div className="absolute top-4 left-4 bg-black/50 backdrop-blur-md p-3 rounded-lg text-sm border border-white/10 pointer-events-none">
          ...
          {/* Elevation View */}
          <div className="flex-1 bg-slate-800 rounded-xl p-4 shadow-2xl border border-slate-700">
            <ElevationMap elevationData={elevationData} />
          </div>
        </div>
